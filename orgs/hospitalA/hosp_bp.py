...
from orgs.hospitalA.access_policy import ORG_POLICIES, DEPT_POLICIES, DEPT_GATE_POLICIES
from orgs.auth_skeleton.models import db, User

ORG = "A"
HOSP_DATA_PATH = "data/"
QDRANT_PATH = "qdrant/"
DEPTS = ["admissions", "psychiatry", "orthopaedics", "surgery", "medicine"]
ROLES = ["physician", "nurse", "admin", "technician", "psychologist"]
...
# Set up router retriever for org with above leaf retrievers as children
...
hosp_bp = Blueprint('hospitalA', __name__)
...
def openid_info():
    config = {
        ...
        "jwks_uri": url_for('hospitalA.jwks_keys', _external=True),
...
@hosp_bp.route('/test/ayjchen', methods=['GET'])
def ayjchen_uri():
    return jsonify(openid_server_uri=current_app.config['OAUTH2_JWT_ISS'])

@hosp_bp.route('/test/dump_users')
def dump_users():
    DUMMY_USERS = [
        {'username': 'sur.p', 'dept': 'surgery', 'role': 'physician'},
        {'username': 'psy.n', 'dept': 'psychiatry', 'role': 'nurse'},
        {'username': 'rad.t', 'dept': 'radiology', 'role': 'technician'},
        {'username': 'staff1', 'dept': 'admissions', 'role': 'admin'},
        {'username': 'neur.p', 'dept': 'neurology', 'role': 'physician'},
        {'username': 'neur.pr', 'dept': 'neurology', 'role': 'physician', 'affiliations': ['C_neuro']},
        {'username': 'neur.r', 'dept': 'neurology', 'role': 'researcher', 'affiliations': ['C_neuro']},
    ]
